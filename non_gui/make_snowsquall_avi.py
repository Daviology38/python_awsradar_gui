...
for file in os.listdir("C:/Users/CoeFamily/radar_data/kbox"):
        ...
        if tstart <= int(time) <= tend:
            try:
                ...
                radar = pyart.io.read_nexrad_archive(
                    "C:/Users/CoeFamily/radar_data/kbox/" + file + "/" + name
                )
                ...
                x0, y0 = m(lon0, lat0)
                glons, glats = m((x0 + x * 1000.0), (y0 + y * 1000.0), inverse=True)
                refl[refl < 15] = np.nan
                refl[refl > 35] = np.nan
    ...
    filenames = natsort.natsorted(
        glob.glob("C:/Users/CoeFamily/Documents/python_awsradar_gui-master/*.png")
    )
    ...
    out = cv2.VideoWriter(date + ".avi", cv2.VideoWriter_fourcc(*"DIVX"), 0.5, size)
